...
<form method="POST" id="updateProductForm" enctype="multipart/form-data">
    ...
    <div class="mb-3">
        ...
        <textarea class="form-control" id="description" name="description" rows="4" required><?= $product['productDescription'] ?></textarea>
    </div>
    <div class="mb-3">
        ...
        <div class="list-images mt-3" id="listImages">
            <?php foreach ($images as $image): ?>
                <div class="image-item position-relative d-inline-block">
                    <img src="/<?= $image['imagePath'] ?>" alt="Image" class="img-thumbnail" style="width: 100px; height: 100px; object-fit: cover;">
                    <button type="button" class="btn btn-sm btn-danger position-absolute top-0 end-0" onclick="removeSelectedImage(this)" data-index="<?= $image['id'] ?>">×</button>
                    </div>
            <?php endforeach; ?>
        </div>             
    </div>
            ...
            <div class="variant-group border p-3 mb-3" id="variant-<?= $index ?>">
                <div class="row">
                    <div class="col-md-2">
                        ...
                        <select name="variants[<?= $index ?>][size]" id="size-<?= $index ?>" class="form-select" required>
                            <option value="">Chọn kích thước</option>
                            <?php foreach ($sizes as $size): ?>
                                <option value="<?= $size['idSize'] ?>" <?= $variant['idSize'] == $size['idSize'] ? 'selected' : '' ?>>
                                    <?= $size['nameSize'] ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="col-md-2">
                        ...
                        <select name="variants[<?= $index ?>][color]" id="color-<?= $index ?>" class="form-select" required>
                            <option value="">Chọn màu sắc</option>
                            <?php foreach ($colors as $color): ?>
                                <option value="<?= $color['idColor'] ?>" <?= $variant['idColor'] == $color['idColor'] ? 'selected' : '' ?>>
                                    <?= $color['nameColor'] ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="col-md-2">
                        ...
                        <input type="number" name="variants[<?= $index ?>][price]" id="price-<?= $index ?>" class="form-control" min="0" value="<?= $variant['price'] ?>" required>
                    </div>
                    <div class="col-md-2">
                        <label for="quantity-<?= $index ?>" class="form-label">Số lượng</label>
                        <input type="number" name="variants[<?= $index ?>][quantity]" id="quantity-<?= $index ?>" class="form-control" min="0" value="<?= $variant['quantityProduct'] ?>" required>
                    </div>
                    <div class="col-md-2">
                        <label for="sku-<?= $index ?>" class="form-label">SKU</label>
                        <input type="text" name="variants[<?= $index ?>][sku]" id="sku-<?= $index ?>" class="form-control" value="<?= htmlspecialchars($variant['sku']) ?>" required>
                    </div>
                    <div class="col-md-2 d-flex align-items-end">
                        <button type="button" class="btn btn-danger" onclick="removeVariant(<?= $index ?>)">Xóa</button>
                    </div>
                </div>
            </div>
        <?php endforeach; ?>
